...
# ==============================================================================
# CRITICAL FIX: PATCH SAM 2 TO SUPPORT PNG / TIF
# ==============================================================================
try:
    ...
    print(torch.cuda.get_device_name(0)) # Should show your RTX 6000 Ada
    import numpy as np
    ...
    # UPDATED SIGNATURE: Added **kwargs to catch 'img_mean', 'img_std', etc.
    def load_video_frames_patched(video_path, image_size, offload_video_to_cpu, async_loading_frames, **kwargs):
        ...
        if os.path.isdir(video_path):
            ...
            # Sort naturally (so frame 10 comes after frame 2)
            try:
...
class SamLabelerApp:
    def __init__(self, parent_frame, on_back):
        ...
        # --- Y-limit Variables ---
        self.y_limit = None # Int value (pixel row). None means disabled.
        self.setting_limit_mode = False # Flag for interaction mode
        self.crosshair_lines = [] # To store canvas IDs
...
# 1. RIGHT SIDE: Auto AI Button
    ...
    def update_presets_2d(self):
        ...
        # 2. Active Class Indicator (Text Only - No Radio Buttons)
    ...
    def auto_segment_2d(self):
        ...
        try:
            # 2. Initialize Generator (Uses existing model)
            ...
            mask_generator = SamAutomaticMaskGenerator(
                ...
                min_mask_region_area=100,  # Minimum pixel area to keep
            )
    ...
    def _build_base_ui(self, title, color):
        ...
        # Added "Edit" mode radio button explicitly
    ...
    def on_mouse_move(self, e):
        ...
        # Draw Semi-Transparent Red Box Preview (Top to Mouse Y)
        # Tkinter doesn't support alpha on shapes natively easily without images, 
        # so we use a stipple pattern to simulate transparency
    ...
    def _run_pv_loop(self):
        ...
        try:
            ...
            # If any error occurs (e.g., user closed window), stop the loop
            self.plotter = None
    ...
    def update_isosurface_3d(self, reset_cam=False):
        ...
        try:
            ...
            # 'name' argument ensures PyVista replaces the old mesh instead of adding a new one
    ...
    def load_current_ct_volume(self):
        ...
        try:
            ...
            # Calculate a step size that forces the 3D model to be small (~256 pixels)
            # This makes a 12GB file use only ~20MB of RAM for the 3D view.
            target_dim = 256
            ...
            # Load the Sparse 3D Preview (This reads quickly and won't crash)
    ...
    def refresh_class_ui(self):
        ...
        # 1. Update the Text Indicator in Toolbar
        ...
        # 2. Update Sidebar Legend (Interactive Selection)
    ...
    def select_model_popup(self):
        ...
        # ======================================================================
        # FIX: DYNAMIC PATH LOGIC (Relative to Script)
        # ======================================================================
        
        # 1. Get the folder where this script (normalize.py) lives
        #    Expected: ...\1. Root folder\1. Python code
        script_dir = os.path.dirname(os.path.abspath(__file__))
        
        # 2. Go up one level to "1. Root folder"
        root_folder = os.path.dirname(script_dir)
        
        # 3. Go down into "models"
        #    Result: ...\1. Root folder\models
        model_folder = os.path.join(root_folder, "models")
        
        # Debugging: Print to console so you can check if it's correct
        print(f"Searching for models in: {model_folder}")

        search_paths = [
            model_folder,                   # Priority 1: The correct relative folder
            os.path.join(os.getcwd(), "models"), # Priority 2: ./models
            os.getcwd()                     # Priority 3: Current folder
        ]
        # ======================================================================
    ...
    def _init_sam(self):
        try:
            ...
            if "sam2" in ckpt_name:
                ...
                # Mapping your specific files to YAML configs
                config_map = {
    ...
    def on_left_click(self, e):
        ...
        if self.mode.get() == "ai":
            ...
            # STEP A: Force Immediate Draw (Show the dot NOW)
            self.show_image(self.raw_image, self.current_mask)
            self.canvas.update_idletasks() # Critical: Forces Tkinter to paint immediately
            
            # STEP B: Run AI (Computer thinks...)
    ...
    def run_sam_prediction(self):
        ...
        try:
            ...
            # 2. EXTRACT THE MASK (This was likely missing)
            raw_mask = masks[0]
    ...
    def show_image(self, rgb, mask=None):
        ...
        # --- NEW: Draw Permanent Red Limit Box on Output Image ---
        if self.y_limit is not None:
...
class YoloTrainerApp:
    def __init__(self, parent_frame, on_back):
        ...
        self.img_size = tk.IntVar(value=640) # RESTORED: Default standard size
        
        # Hidden variable: defaults to False (Train on ALL classes)
    ...
    def setup_ui(self):
        ...
        # --- Config Row (Restored Img Size, Removed Keyhole Checkbox) ---
...
class TrackingViewer:
    def __init__(self, master):
        # Use Toplevel to ensure it opens as a separate, persistent window
        ...
        try:
            ...
            if boxes is not None:
                for i, box in enumerate(boxes):
                    ...
                    # Find best Keyhole (Class 0)
                    if cls_id == 0 and conf > best_conf:
                        best_conf = conf
                        best_box = box.xywh[0] # center_x, center_y, w, h
                        
                        # --- NEW: Extract Polygon Logic ---
                        if masks is not None:
                            # masks.xy is a list of arrays. We need the one at index i.
                            # It returns pixels: [[x1, y1], [x2, y2], ...]
                            poly_coords = masks.xy[i]
                            
                            # Convert numpy array to a standard list so it saves cleanly to CSV
                            # result: "[[10.5, 20.1], [11.0, 20.5], ...]"
    ...
    def update_canvas(self, cv_img):
        ...
        # Resize logic to fit window (Fixed height 700px)
        disp_h = 700
        scale = disp_h / h
        disp_w = int(w * scale)
